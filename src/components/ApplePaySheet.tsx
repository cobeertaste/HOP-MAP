...
export default function ApplePaySheet({ itemName, price, lang = 'PT', onConfirm, onClose }: ApplePaySheetProps) {
        ...
        <AnimatePresence>
          {purchaseSuccess ? (
            ...
            >
              ...
              <div>
                <h4 className="text-xl font-bold">
                  {lang === 'EN' ? 'Payment Successful!' : 'Pago com Sucesso!'}
                </h4>
                <p className="text-zinc-400 text-xs mt-1">
                  {lang === 'EN' ? 'Your ticket was added to your Hop-Map Wallet.' : 'O teu bilhete foi adicionado à Carteira Hop-Map.'}
                </p>
              </div>
            </motion.div>
          ) : (
            <div className="space-y-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-xs text-zinc-400">
                      {lang === 'EN' ? 'Festival / Special Event' : 'Festival / Evento Especial'}
                    </p>
              ...
              {/* Cards list */}
              <div>
                <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest pl-1 mb-2">
                  {lang === 'EN' ? 'Payment Card' : 'Cartão de Pagamento'}
                </p>
                <div className="space-y-2">
                  <button 
                    onClick={() => setSelectedCard('apple')}
                    className={`w-full flex items-center justify-between p-3.5 rounded-xl border transition cursor-pointer ${
                      selectedCard === 'apple' 
              ...
              <div className="pt-2 text-zinc-400 text-xs space-y-1.5">
                ...
                <div className="flex justify-between pl-1">
                  <span>{lang === 'EN' ? 'Official Glass Kit & Fees' : 'Kit de Copo Oficial & Taxas'}</span>
                  <span className="text-white">1.50€</span>
                </div>
                <div className="flex justify-between pl-1 border-t border-zinc-800 pt-2 font-semibold">
                  <span className="text-zinc-300">{lang === 'EN' ? 'Total Amount' : 'Valor Total'}</span>
              ...
              <div className="pt-4">
                {isProcessing ? (
                  <button 
                    disabled
                    className="w-full h-12 rounded-xl bg-zinc-800 text-zinc-400 font-bold flex items-center justify-center space-x-2 cursor-wait"
                  >
                    <svg className="animate-spin h-5 w-5 text-amber-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span>{lang === 'EN' ? 'Securing connection...' : 'Segurando ligação segura...'}</span>
                  </button>
                ) : (
                  <button 
                    onClick={handlePayClick}
                    className="w-full h-12 rounded-xl bg-white text-zinc-950 font-bold flex items-center justify-center space-x-2 shadow-lg hover:bg-zinc-100 transition active:scale-95 duration-100 cursor-pointer"
                    id="btn-confirm-applepay"
                  >
                    <CreditCard className="w-5 h-5 text-black" />
                    <span>{lang === 'EN' ? 'Confirm with  Pay Biometrics' : 'Confirmar com Biometria  Pay'}</span>
                  </button>
                )}
                <p className="text-[10px] text-zinc-500 text-center mt-3">
                  {lang === 'EN' ? 'End-to-end encrypted secure transaction certified for Cobeer Taste.' : 'Transação segura encriptada fim-a-fim certificada para Cobeer Taste.'}
                </p>
              </div>
            </div>
          )}
