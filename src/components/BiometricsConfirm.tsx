...
export default function BiometricsConfirm({ onSuccess, onCancel, reason, lang = 'PT' }: BiometricsConfirmProps) {
    ...
    <div className="absolute inset-0 bg-black/85 backdrop-blur-md flex flex-col items-center justify-center p-6 z-[200]">
      ...
      >
        ...
        <h3 className="text-white font-semibold text-lg tracking-tight font-sans">
          {scanState === 'scanning' ? 'Face ID' : (lang === 'EN' ? 'Processed' : 'Processado')}
        </h3>
        
        <p className="text-zinc-400 text-xs mt-2 leading-relaxed min-h-[32px] font-sans">
          {scanState === 'scanning' ? reason : (lang === 'EN' ? 'Authentication successful' : 'Autenticação bem-sucedida')}
        </p>
        ...
        {scanState === 'scanning' && (
          <button 
            onClick={onCancel}
            className="mt-6 text-zinc-500 hover:text-zinc-300 text-xs font-semibold py-1 px-4 transition underline underline-offset-4 cursor-pointer"
