...
type BadgeFilterCategory = 'all' | 'checkins' | 'spots' | 'regions' | 'community' | 'styles' | 'time' | 'holidays' | 'special';

// Exact priority requested for categories:
// 1. "CHECK-INS" ('checkins')
// 2. "SPOTS" ('spots')
// 3. "Regiões" ('regions')
// 4. "COMUNIDADE" ('community')
// 5. "Estilos" ('styles')
// 6. "Horários" ('time')
// 7. "DIAS FESTIVOS" ('holidays')
const CATEGORY_ORDER: Record<string, number> = {
  ...
  time: 6,
  holidays: 7,
  special: 8, // fallback/special badges
};
...
export function BadgesModal({
  ...
  // Badges sorted by:
  // 1. Unlocked (conquistados) first
  // 2. Category order: checkins -> spots -> regions -> community -> styles -> time -> holidays
  // 3. Alphabetical by localized name (PT or EN)
  // Must be declared at the top level before any conditional returns to obey React Rules of Hooks
  const filteredBadges = useMemo(() => {
    ...
    return list.sort((a, b) => {
      // 1. Conquered/unlocked badges first
      if (a.unlocked && !b.unlocked) return -1;
      if (!a.unlocked && b.unlocked) return 1;

      // 2. Category priority
      const catOrderA = CATEGORY_ORDER[a.badge.category] ?? 99;
      const catOrderB = CATEGORY_ORDER[b.badge.category] ?? 99;
      if (catOrderA !== catOrderB) {
        return catOrderA - catOrderB;
      }

      // 3. Alphabetical by localized name (PT or EN)
      const nameA = (lang === 'PT' ? a.badge.namePt : a.badge.nameEn) || '';
      const nameB = (lang === 'PT' ? b.badge.namePt : b.badge.nameEn) || '';
      return nameA.localeCompare(nameB, lang === 'PT' ? 'pt' : 'en', { sensitivity: 'base' });
    });
  }, [badgeStatuses, activeCategory, lang]);
