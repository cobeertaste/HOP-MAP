...
import { formatRelativeTime } from './lib/socialActivity';
          ...
          try {
            ...
            querySnapshot.forEach((docSnap) => {
              ...
              details.push({
                ...
                checkedInBars: Array.isArray(data.checkedInBars) ? data.checkedInBars : [],
  ...
  useEffect(() => {
    ...
    const fetchFriendCheckins = async () => {
      try {
        ...
        // Also query real check-ins from 'checkins' collection
        try {
          const chkQ = query(
          ...
          const chkSnapshot = await getDocs(chkQ);
          chkSnapshot.forEach((docSnap) => {
            const chkData = docSnap.data();
            if (chkData.userId && chkData.userId !== user.id) {
              const ts = chkData.timestamp?.toMillis 
                ? chkData.timestamp.toMillis() 
                : (chkData.timestamp ? new Date(chkData.timestamp).getTime() : Date.now());
              freshActivities.push({
                id: `social_chk_${docSnap.id}`,
                friendId: chkData.userId,
                friendUsername: chkData.username || 'Amigo Cervejeiro',
                type: 'checkin',
                spotId: chkData.spotId,
                spotName: chkData.spotName,
                timestamp: ts,
                relativeTimePt: formatRelativeTime(ts, 'PT'),
                relativeTimeEn: formatRelativeTime(ts, 'EN')
              });
            }
          });
        } catch (chkErr) {
          // If compound query or index is not set up, query without ordering
          try {
            const chkQFallback = query(collection(db, 'checkins'), limit(25));
            const chkSnapshot = await getDocs(chkQFallback);
            chkSnapshot.forEach((docSnap) => {
              const chkData = docSnap.data();
              if (chkData.userId && chkData.userId !== user.id) {
                const ts = chkData.timestamp?.toMillis 
            ...
            console.warn("Could not query fallback checkins:", fbErr);
          }
        }
  ...
  return (
    <AppleDeviceFrame darkMode={darkMode} setDarkMode={setDarkMode}>
      ...
      <div className="flex-1 w-full h-full flex flex-col relative overflow-hidden bg-[#F6EFDC] text-[#1B2036]">
        ...
        {user.isLoggedIn && (
          <header className="px-2.5 sm:px-4 py-2 sm:py-2.5 flex items-center justify-between border-b-2 border-[#1B2036] bg-[#F6EFDC] text-[#1B2036] z-30 shrink-0 select-none w-full sticky top-0 landscape-compact-header pt-[max(0.5rem,env(safe-area-inset-top,0px))]">
            ...
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              {/* Dropdown Menu (3 horizontal bars) with Perfil, HOP-Chat, Oferece uma rodada, Classificações and Ajuda & FAQ */}
              <HeaderDropdownMenu
                lang={lang}
                onOpenProfile={() => {
                  playPacmanSound();
                ...
                onOpenHopChat={() => setIsHopChatOpen(true)}
                onOpenDonation={() => setIsDonationModalOpen(true)}
                onOpenLeaderboard={() => setShowScoreModal(true)}
                ...
                <div className="p-3.5 sm:p-4 rounded-2xl border-2 border-zinc-700 bg-[#FAF6EB] shadow-[3px_3px_0px_#1B2036]">
                  <SocialActivitySection
                    ...
                    onViewFriendProfile={(friend) => {
                      const fullFriend = friendsDetails.find(f => f.id === friend.id) || friend;
                      setSelectedFriendProfile(fullFriend);
                    }}
                ...
                <div className="p-4 rounded-2xl space-y-3 border-2 border-zinc-700 transition-all bg-[#F6EFDC] text-neutral-900 shadow-xs">
                  <div className="flex items-center justify-between gap-3">
                    <div className="pr-2">
                      ...
                      <p className="text-[9px] mt-0.5 leading-normal text-zinc-600">
                        {lang === 'PT'
                          ? 'Dúvidas sobre check-ins, pontuação HOPS, selos e regras da comunidade.'
                          : 'Questions about check-ins, HOPS scoring, stamps, and community guidelines.'}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsHelpFaqOpen(true)}
                      className="px-3 py-2 rounded-xl text-[10px] font-bold font-display bg-[#12908C] hover:bg-[#107b77] active:scale-95 text-white transition-all shadow-xs cursor-pointer shrink-0 border border-zinc-700 flex items-center gap-1.5"
                      id="btn-open-help-faq-profile"
                    >
                      <span>{lang === 'PT' ? 'Consultar' : 'Open FAQ'}</span>
                    </button>
                  </div>
                </div>
        ...
        {user.isLoggedIn && (
          <nav 
            className="border-t-2 border-[#1B2036] bg-[#F6EFDC] shrink-0 sticky bottom-0 flex items-center justify-around px-2 sm:px-4 z-[160] select-none w-full max-w-full overflow-hidden landscape-compact-nav bottom-nav-safe"
            style={{
          ...
          >
            ...
            >
              <div className={`p-0.5 rounded-md transition-all ${activeTab === 'explore' ? 'scale-110' : 'opacity-75'}`}>
                <PixelIcon name="compass" size={19} overrideColor={activeTab === 'explore' ? "#12908C" : "#1B2036"} />
              </div>
              <span className={`text-[8px] sm:text-[9px] font-press uppercase tracking-wider truncate max-w-full ${activeTab === 'explore' ? 'text-[#12908C] font-bold' : 'text-[#1B2036] font-bold opacity-80'}`}>
                {t('tabMap', lang)}
              </span>
            </button>
            ...
            >
              <div className={`p-0.5 rounded-md transition-all ${activeTab === 'routes' ? 'scale-110' : 'opacity-75'}`}>
                <PixelIcon name="route" size={19} overrideColor={activeTab === 'routes' ? "#12908C" : "#1B2036"} />
              </div>
              <span className={`text-[8px] sm:text-[9px] font-press uppercase tracking-wider truncate max-w-full ${activeTab === 'routes' ? 'text-[#12908C] font-bold' : 'text-[#1B2036] font-bold opacity-80'}`}>
                {t('tabRoutes', lang)}
              </span>
            </button>
            ...
            >
              <div className={`p-0.5 rounded-md transition-all ${activeTab === 'events' ? 'scale-110' : 'opacity-75'}`}>
                <PixelIcon name="calendar" size={19} overrideColor={activeTab === 'events' ? "#12908C" : "#1B2036"} />
              </div>
              <span className={`text-[8px] sm:text-[9px] font-press uppercase tracking-wider truncate max-w-full ${activeTab === 'events' ? 'text-[#12908C] font-bold' : 'text-[#1B2036] font-bold opacity-80'}`}>
                {t('tabFestivals', lang)}
              </span>
            </button>
            ...
            >
              <div className={`p-0.5 rounded-md transition-all ${activeTab === 'loyalty' ? 'scale-110' : 'opacity-75'}`}>
                <PixelIcon name="ticket" size={19} overrideColor={activeTab === 'loyalty' ? "#12908C" : "#1B2036"} />
              </div>
              <span className={`text-[8px] sm:text-[9px] font-press uppercase tracking-wider truncate max-w-full ${activeTab === 'loyalty' ? 'text-[#12908C] font-bold' : 'text-[#1B2036] font-bold opacity-80'}`}>
                {t('tabCheckin', lang)}
              </span>
            </button>
          </nav>
