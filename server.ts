...
async function startServer() {
  ...
  // Explicitly serve local spot photos and public static assets
  const publicDir = path.resolve(process.cwd(), 'public');
  const spotsPicsDir = path.join(publicDir, 'spots_pics');
  app.use('/shots_pics', express.static(spotsPicsDir, { maxAge: '30d' }));
  app.use('/spots_pics', express.static(spotsPicsDir, { maxAge: '30d' }));
